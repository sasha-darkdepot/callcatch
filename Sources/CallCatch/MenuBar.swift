...
final class MenuBar: NSObject {
    ...
    private let autoStopItem = NSMenuItem(title: "Auto-stop", action: #selector(autoStopTapped), keyEquivalent: "")
    ...
    init(settings: Settings,
        ...
        menu.addItem(quit)
        [findIdItem, recordItem, stopItem, autoItem, autoStopItem, loginItem].forEach { $0.target = self }
        ...
        autoStopItem.toolTip = "When a recording started automatically, stop it 10 s after CallCatch detects the call end. Hover the bubble to pause the countdown."
        item.menu = menu
    ...
    func update(status: MenuStatus, canRecordNow: Bool, canStopNow: Bool) {
        ...
        autoStopItem.state = settings.autoStop ? .on : .off
    ...
    @objc private func autoStopTapped() {
        settings.autoStop.toggle()
        autoStopItem.state = settings.autoStop ? .on : .off
        onAutoStopToggled() // FSM гасит живой отсчёт, если тумблер выключили
    }
